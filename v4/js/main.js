...
$('#sliderContainer').serialScroll({
    ...
    function update() {
    	...
    	if(sumCircles)
    	{
    		...
    		//select user dot
    		svg.select("circle.user").attr("class", "remove");
    ...
    function draw() {	    	
		...
		if(dotsExpanded){
			...
			//if dots are expanded, we need to transition them to be visible	
		    posNegCircles
		    	.attr("cx", function(d) { return xScale(d[0]); })
		    	.attr("cy", function(d) { return yScale(d[1]); })
			    .attr("fill", function(d) { return color(d[0]); })
	    	...
	    	posNegCircles.transition()
				...
				.delay(function(d, i) { return i * delayFactor + shortAnimation; }) // need shortAnimation to take delay until the remove is done
				.ease("bounce")
		...
		if (userValue) {
			...
			quizSum = svg.selectAll("circle.user")
				.data(userValue)
				.enter().append("circle")
				.attr("class", "user")
				.attr("cx", function (d) { return xScale(d[0]); })
	...
	function mouseover(d, i){
		...
		if(!dotsExpanded)
		{
			...
			posDot
				...
				.ease("elastic", 1 + (posNegData[curQuestion][i][1]/2), .6)
